...
//==================== SOLVE ======================//
string mergeSubsequence(string ans, string s) {
    string res = "";
    int i = 0, j = 0;
...
void solve() {
    ...
    for (auto &p : freq) {
        for (int i = 0; i < p.second; i++) {
            requiredChars.push_back(p.first);
        }
    }
    sort(all(requiredChars));
    string ans = "";
    ...
    int n = sz(ans) + sz(s);
    for(int i = 0; i < n; i++) {
        if(i == sz(ans) || ans[i] >= startChar){
            ans = mergeSubsequence(ans, s);
            cout << ans << "\n";
            return;
        }
    }
